...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    add_source(p)
    p.add_argument(
        "--srt", default=None, help="Path to translated SRT (overrides auto-translate)."
    )
    p.add_argument(
        "--original-audio", default=None, help="Original audio for duration matching."
    )
    add_voice(p)
    ...
    p.add_argument(
        "--dtype", default="bfloat16", help="Weight dtype (for Qwen engine)."
    )
    add_tempo(p)
    add_llm(p)
    add_transcription(p)
...
def handler(args: argparse.Namespace) -> None:
    ...
    if not srt_path and proj:
        ...
        if not os.path.exists(proj.final_srt):
            ...
            with open(proj.source_srt, encoding="utf-8") as fh:
                srt_text = fh.read()
            description = (
                load_json(proj.description) if os.path.exists(proj.description) else {}
            )
            thumb_paths = (
                load_json(proj.thumbs_meta) if os.path.exists(proj.thumbs_meta) else []
            )
            translated = translate_srt(
                ...
                **(
                    dict(words_per_second=args.words_per_second)
                    if args.words_per_second is not None
                    else {}
                ),
                **(
                    dict(duration_budget=args.duration_budget)
                    if args.duration_budget is not None
                    else {}
                ),
            )
    ...
    segments_dir = args.segments_dir or (
        proj.tts_segments_dir if proj else "./tts_segments"
    )
    ...
    if voice_theme and proj:
        ...
        device = (
            args.device.split(":")[0] + ":0" if ":" not in args.device else args.device
        )
