...
# [mazinger] Deferred so that importing this module -- which the 12 Hz model
# does transitively -- no longer requires `sox` and `onnxruntime`.  See the
# note in ``qwen_tts/core/__init__.py``.
def _register_25hz() -> bool:
    """Register the 25 Hz tokenizer with the Auto* classes, if importable."""
    from transformers import AutoConfig, AutoModel

    try:
        from ..core import Qwen3TTSTokenizerV1Config, Qwen3TTSTokenizerV1Model
    except ImportError as exc:
        logger.debug("25 Hz tokenizer unavailable (%s); 12 Hz models unaffected", exc)
        return False
...
class Qwen3TTSTokenizer:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, **kwargs) -> "Qwen3TTSTokenizer":
        ...
        _register_25hz()  # [mazinger] no-op when sox/onnxruntime are absent
