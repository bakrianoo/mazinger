...
# [mazinger] Renamed from ...DecoderRotatoryEmbedding ("Rotatory" is an upstream
# typo).  transformers 5.x re-materialises rotary buffers only for modules whose
# class name contains the literal substring "RotaryEmbedding", so the misspelling
# silently excluded this module and left its inv_freq all-zeros.  An alias under
# the original name is kept below for compatibility.
...
# [mazinger] backwards-compatible alias for the original (misspelled) name.
Qwen3TTSTokenizerV2DecoderRotatoryEmbedding = Qwen3TTSTokenizerV2DecoderRotaryEmbedding
...
class Qwen3TTSTokenizerV2DecoderTransformerModel(Qwen3TTSTokenizerV2DecoderPreTrainedModel):
    ...
    @check_model_inputs  # [mazinger] plain decorator in transformers 5.x
    @auto_docstring
    def forward(
        self,
        input_ids=None,
        ...
        **kwargs,
        ...
        if not isinstance(causal_mask_mapping := attention_mask, dict):
            ...
            mask_kwargs = {
                ...
                "attention_mask": attention_mask,
                # [mazinger] cache_position dropped from the 5.x signature
                "past_key_values": past_key_values,
                "position_ids": position_ids,
            }
            # Create the masks
