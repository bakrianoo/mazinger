...
# [mazinger] transformers 5.x removed the "default" entry from
# ROPE_INIT_FUNCTIONS (unscaled RoPE now goes through config.rope_parameters),
# but this model looks it up whenever rope_scaling is unset.  Ported verbatim
# from transformers 4.57.3 -- the version upstream pins -- so checkpoints keep
# the exact frequencies they were trained with.
...
class Qwen3TTSPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        # [mazinger] see the note on Qwen3TTSTalkerTextPreTrainedModel._init_weights
...
class Qwen3TTSTalkerTextPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        # [mazinger] transformers 5.x builds the model on the meta device and then
        # rebuilds every non-persistent buffer through PreTrainedModel._init_weights.
        # `inv_freq` is such a buffer, and the base implementation is the only thing
        # that restores it -- but this override replaces the base entirely, so under
        # 5.x inv_freq stayed all-zeros and RoPE collapsed to cos=1/sin=0, i.e. the
        # model ran with no positional information.  Nothing raises; output quality
        # just falls apart and generation never emits EOS.  Delegating the rotary
        # case upward restores it while leaving the weight init below untouched.
        if "RotaryEmbedding" in module.__class__.__name__:
            return super()._init_weights(module)
...
class Qwen3TTSTalkerRotaryEmbedding(nn.Module):
    def __init__(self, config: Qwen3TTSTalkerConfig, device=None):
        ...
        self.config = config
        self.rope_init_fn = ROPE_INIT_FUNCTIONS.get(  # [mazinger] .get + fallback
            self.rope_type, _compute_default_rope_parameters
        )

        inv_freq, self.attention_scaling = self.rope_init_fn(self.config, device)
        self.register_buffer("inv_freq", inv_freq, persistent=False)
        # [mazinger] transformers 5.x materialises models on the meta device and
        # then re-initialises non-persistent buffers -- inv_freq among them -- in
        # PreTrainedModel._init_weights.  That code path requires the buffer to be
        # registered (not just assigned) *and* the module to expose
        # `compute_default_rope_parameters`; upstream has neither, so inv_freq was
        # left as all-zeros.  RoPE then degenerated to cos=1/sin=0 -- no positional
        # information at all -- which does not raise anywhere, it just silently
        # destroys output quality.  See `compute_default_rope_parameters` below.
        self.register_buffer("original_inv_freq", inv_freq.clone(), persistent=False)

    # [mazinger] hook required by transformers 5.x `_init_weights`; it is called
    # as `module.compute_default_rope_parameters(module.config)`.
...
class Qwen3TTSTalkerCodePredictorModel(Qwen3TTSPreTrainedModel):
    ...
    def __init__(self, config: Qwen3TTSTalkerCodePredictorConfig, embedding_dim: int):
        ...
        self.padding_idx = getattr(config, "pad_token_id", None)  # [mazinger] 5.x moved token ids to GenerationConfig
    ...
    def forward(
        ...
        **flash_attn_kwargs,
        ...
        if not isinstance(causal_mask_mapping := attention_mask, dict):
            ...
            mask_kwargs = {
                "config": self.config,
                "inputs_embeds": inputs_embeds,  # [mazinger] renamed in 5.x
                "attention_mask": attention_mask,
                "past_key_values": past_key_values,
                # [mazinger] 5.x renamed input_embeds and dropped cache_position
                "position_ids": position_ids,
            }
            # Create the masks
            causal_mask_mapping = {
...
class Qwen3TTSTalkerForConditionalGeneration(Qwen3TTSTalkerTextPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # [mazinger] transformers 5.x stopped threading `cache_position` into
        # forward(), so it arrives as None on *every* step -- including decode.
        # The branch below reads it to tell prefill from decode; left as None it
        # always takes the prefill path and rebuilds full-length position_ids for
        # a single-token step.  That does not raise where it happens: RoPE simply
        # broadcasts the 1-token query up to the full sequence length, and the
        # mismatch only surfaces later as a mask/key length error in attention.
        # `cache_position[0]` is just the number of already-cached tokens, so
        # rebuilding it from the cache restores the original semantics exactly.
        if cache_position is None and past_key_values is not None:
            past_len = past_key_values.get_seq_length()
            step_len = inputs_embeds.shape[1] if inputs_embeds is not None else input_ids.shape[1]
            cache_position = torch.arange(
                past_len, past_len + step_len, device=inputs_embeds.device,
            )
